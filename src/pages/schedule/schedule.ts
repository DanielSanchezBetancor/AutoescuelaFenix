...
@Component({
  ...
  "{{dt.day}} - {{dt.date | date: 'dd-MM-yyyy'}}" + 
...
export class SchedulePage {
	...
	createDates() {
		...
		for (var i = 0;i<5;i++) {
			...
			var day = today.toString().substring(0,3);
			if (day == "Mon") {
				DT.day = "Lunes";
			} else if (day == "Tue") {
				DT.day = "Martes";
			...
			console.log(DT.date);
			DT.firsth = this.calculateHour(DT.date, this.hours[0]);
			DT.secondh = this.calculateHour(DT.date, this.hours[1]);
			DT.thirdh = this.calculateHour(DT.date, this.hours[2]);
			DT.fourh = this.calculateHour(DT.date, this.hours[3]);
			this.dates[i] = DT;
	...
	showConfirmDialog() {
		...
		for (var i = 0;i<this.dates.length;i++) {
			alert.addInput({
				type: 'radio',
				label: this.days[i] + " - " + this.dates[i].date,
